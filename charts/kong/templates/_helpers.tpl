...
{{/* TODO: the rm command here is a workaround for https://github.com/Kong/charts/issues/295
  ...
  command: [ "/bin/sh", "-c", "until kong start; do echo 'waiting for db'; sleep 1; done; kong stop; if stat {{ $sockFile }} > /dev/null; then rm {{ $sockFile }}; echo '{{ $sockFile }} removed'; else echo '{{ $sockFile }} noop'; ls {{ .Values.env.prefix }}; fi"]
  volumeMounts:
